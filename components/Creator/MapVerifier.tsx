...
import { Search, MapPin, X, Save, Trash2, Check, Loader2 } from 'lucide-react';
...
const MapVerifier: React.FC<MapVerifierProps> = ({
  points,
  setPoints,
  onSaveAndContinue,
  isPublishing,
  ...
  }, [map, points]); // Added points to dependencies
  ...
  const _handleMapClick = (e: google.maps.MapMouseEvent) => {
  ...
  const handleSaveEdit = () => {
    setPoints((prev) =>
      prev.map((p) => (p.id === selectedPointId ? ({ ...p, ...editForm } as TripPoint) : p))
    );
    setIsDirty(false);
  };
                  ...
                  <div className="flex gap-2 flex-wrap">
                    {POINT_TYPES.map((type) => (
                      <button
                        key={type}
                        onClick={() => {
                          setEditForm((prev) => ({ ...prev, type: type as PointType }));
                          setIsDirty(true);
                        }}
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        setEditForm((prev) => ({ ...prev, day: parseInt(e.target.value) }));
                        setIsDirty(true);
                      }}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        setEditForm((prev) => ({ ...prev, timestamp: parseInt(e.target.value) }));
                        setIsDirty(true);
                      }}
                      className="w-full bg-white border border-gray-200 rounded-lg p-3 font-bold text-sm outline-none focus:border-black transition"
                    />
                ...
                {points.map((point) => (
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      <h4 className="font-bold text-sm text-gray-900 truncate group-hover:text-black">
                        {point.name}
                      </h4>
