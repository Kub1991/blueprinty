...
export function useCreatorVideos() {
  ...
  const myBlueprints = useQuery(
    api.blueprints.listByUser,
    currentUserId ? { userId: currentUserId } : 'skip'
  );
  ...
  const handleDisconnect = async () => {
    ...
    if (
      confirm(
        'Czy na pewno chcesz odłączyć kanał i usunąć zaimportowane filmy? Plany podróży pozostaną nienaruszone.'
      )
    ) {
      try {
