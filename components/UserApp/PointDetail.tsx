...
const PointDetail: React.FC<PointDetailProps> = ({ point, blueprint, onBack }) => {
  ...
  React.useEffect(() => {
    ...
    const fetchWithModernSDK = async (id: string) => {
      try {
        ...
        svc.getDetails(
          { placeId: id, fields: ['photos', 'html_attributions'] },
          (result, status) => {
            if (status === google.maps.places.PlacesServiceStatus.OK && result?.photos?.[0]) {
              ...
              if (
                result.photos[0].html_attributions &&
                result.photos[0].html_attributions.length > 0
              ) {
                // Extract text from HTML attribution
                const parser = new DOMParser();
                const doc = parser.parseFromString(
                  result.photos[0].html_attributions[0],
                  'text/html'
                );
  ...
  console.log(
    `[PointDetail-V6.2] Rendering: ${point.name}. Source: ${fetchedImageUrl ? 'Modern-SDK' : point.photoReference ? 'Legacy-Ref' : 'Static'}`
  );
      ...
      <div className="p-8 space-y-8">
        ...
        {(() => {
          ...
          if (!blueprint.youtubeVideoId) {
            console.warn(
              `[PointDetail] YouTube Video ID missing for blueprint: ${blueprint.title}`
            );
          }

          const videoId =
            blueprint.youtubeVideoId ||
            (blueprint.title === 'Chodzimy po Mogadiszu' ? 'EtdLwsV-VP8' : null);
