...
export const fetchChannelVideos = action({
    ...
    handler: async (ctx, args): Promise<{ success: boolean; count: number; hasMore: boolean }> => {
        ...
        console.log(`[ytProcessor] Fetching videos for channel: ${args.channelHandle}${pageToken ? ` (Page: ${pageToken})` : ''}`);
        ...
        try {
            ...
            for (const poi of rawPOIs) {
                ...
                enrichedPoints.push({
                    ...
                    type: (poi.type || "activity") as any,
                    day: poi.day || 1,
                    timestamp: typeof poi.timestamp === 'number' ? poi.timestamp : parseInt(String(poi.timestamp || 0)) || 0,
            ...
            console.error("[ytProcessor] Processing failed:", error.message);
